...
interface InstantOptimizationCompleteProps {
  originalPrompt: string
  optimizedPrompt: string
  onNewOptimization?: () => void
}

export default function InstantOptimizationComplete({ 
  originalPrompt, 
  optimizedPrompt,
  onNewOptimization
}: InstantOptimizationCompleteProps) {
  ...
  // 计算优化提升
  const improvement = Math.round((optimizedPrompt.length / originalPrompt.length - 1) * 100)
    ...
    <div className="w-full max-w-6xl mx-auto">
      ...
      <div className="text-center mb-8 animate-fade-in">
        <h2 className="text-3xl font-extralight text-white mb-2">
          提示词优化提升了 <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-400">{Math.abs(improvement)}%</span> 的效果
        </h2>
      </div>
